import React from 'react'

const BuildCircleIcon = () => {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      height="32px"
      width="32px"
      viewBox="0 0 40 40"
    >
      <path
        fill="#1848C2"
        d="m26.875 28.5 1.375-1.375q.25-.208.25-.521 0-.312-.25-.521l-6.5-6.541q.25-.542.396-1.125.146-.584.146-1.292 0-2.208-1.584-3.771-1.583-1.562-3.791-1.562-.625 0-1.167.146-.542.145-1.042.395l3.375 3.417-2.541 2.5-3.459-3.375q-.25.5-.375 1.083-.125.584-.125 1.167 0 2.25 1.542 3.792 1.542 1.541 3.792 1.541.666 0 1.25-.146.583-.145 1.166-.395l6.5 6.541q.25.209.542.23.292.02.5-.188ZM20 35q-3.083 0-5.813-1.188-2.729-1.187-4.77-3.229-2.042-2.041-3.229-4.75Q5 23.125 5 20t1.188-5.854q1.187-2.729 3.229-4.75 2.041-2.021 4.75-3.208Q16.875 5 20 5t5.854 1.188q2.729 1.187 4.75 3.208t3.208 4.75Q35 16.875 35 20q0 3.083-1.188 5.812-1.187 2.73-3.208 4.771-2.021 2.042-4.75 3.229Q23.125 35 20 35Z"
      />
    </svg>
  )
}

const SearchIcon24 = () => {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" height="24" width="24">
      <path d="m19.475 20.15-6.25-6.25q-.75.625-1.725.975-.975.35-1.95.35-2.425 0-4.087-1.663Q3.8 11.9 3.8 9.5q0-2.4 1.663-4.063 1.662-1.662 4.062-1.662 2.4 0 4.075 1.662Q15.275 7.1 15.275 9.5q0 1.05-.375 2.025-.375.975-.975 1.65L20.2 19.45ZM9.55 14.225q1.975 0 3.35-1.362Q14.275 11.5 14.275 9.5T12.9 6.137q-1.375-1.362-3.35-1.362-2 0-3.375 1.362Q4.8 7.5 4.8 9.5t1.375 3.363q1.375 1.362 3.375 1.362Z" />
    </svg>
  )
}

const Schedule32 = () => {
  return (
    <svg
      height="24px"
      width="24px"
      viewBox="0 0 24 24"
      data-testid='ScheduleIcon'
    >
      <path
        fill='#ABC990'
        d='M11.99 2C6.47 2 2 6.48 2 12s4.47 10 9.99 10C17.52 22 22 17.52 22 12S17.52 2 11.99 2zM12 20c-4.42 0-8-3.58-8-8s3.58-8 8-8 8 3.58 8 8-3.58 8-8 8z'
      />
      <path fill='#789770' d='M12.5 7H11v6l5.25 3.15.75-1.23-4.5-2.67z' />
    </svg>
  );
}

const CodeSnippet32 = () => {
  return (
    <svg
      height="24px"
      width="24px"
      viewBox='0 0 24 24'
      data-testid='CodeIcon'
    >
      <path fill='#CBA911' d='M9.4 16.6 4.8 12l4.6-4.6L8 6l-6 6 6 6 1.4-1.4zm5.2 0 4.6-4.6-4.6-4.6L16 6l6 6-6 6-1.4-1.4z'></path>
    </svg>
  );
}

const Bookmarks32 = () => {
  return (
    <svg
      height="24px"
      width="24px"
      viewBox='0 0 24 24'
      data-testid='BookmarksIcon'
    >
      <path fill='#00CCDD' d='m19 18 2 1V3c0-1.1-.9-2-2-2H8.99C7.89 1 7 1.9 7 3h10c1.1 0 2 .9 2 2v13zM15 5H5c-1.1 0-2 .9-2 2v16l7-3 7 3V7c0-1.1-.9-2-2-2z'></path>
    </svg>
  );
}

const Notification32 = () => {
  return (
    <svg
      height="24px"
      width="24px"
      viewBox='0 0 24 24'
      data-testid='NotificationsActiveRoundedIcon'
    >
      <path fill='#DD5555' d='M18 16v-5c0-3.07-1.64-5.64-4.5-6.32V4c0-.83-.68-1.5-1.51-1.5S10.5 3.17 10.5 4v.68C7.63 5.36 6 7.92 6 11v5l-1.3 1.29c-.63.63-.19 1.71.7 1.71h13.17c.89 0 1.34-1.08.71-1.71L18 16zm-6.01 6c1.1 0 2-.9 2-2h-4c0 1.1.89 2 2 2zM6.77 4.73c.42-.38.43-1.03.03-1.43-.38-.38-1-.39-1.39-.02C3.7 4.84 2.52 6.96 2.14 9.34c-.09.61.38 1.16 1 1.16.48 0 .9-.35.98-.83.3-1.94 1.26-3.67 2.65-4.94zM18.6 3.28c-.4-.37-1.02-.36-1.4.02-.4.4-.38 1.04.03 1.42 1.38 1.27 2.35 3 2.65 4.94.07.48.49.83.98.83.61 0 1.09-.55.99-1.16-.38-2.37-1.55-4.48-3.25-6.05z'></path>
    </svg>
  );
}

const CalendarToday32 = () => {
  return (
    <svg
      height="24px"
      width="24px"
      viewBox='0 0 24 24'
      data-testid='TodayIcon'
    >
      <path fill='#5555DD' d='M19 3h-1V1h-2v2H8V1H6v2H5c-1.11 0-1.99.9-1.99 2L3 19c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm0 16H5V8h14v11zM7 10h5v5H7z'></path>
    </svg>
  );
}

const Satellite32 = () => {
  return (
    <svg
      height="24px"
      width="24px"
      viewBox='0 0 24 24'
      data-testid='SatelliteAltIcon'
    >
      <path fill='#FF885E' d='m15.44.59-3.18 3.18c-.78.78-.78 2.05 0 2.83l1.24 1.24-.71.71-1.24-1.25c-.78-.78-2.05-.78-2.83 0L7.3 8.72c-.78.78-.78 2.05 0 2.83l1.24 1.24-.71.71-1.23-1.25c-.78-.78-2.05-.78-2.83 0L.59 15.43c-.78.78-.78 2.05 0 2.83l3.54 3.54c.78.78 2.05.78 2.83 0l3.18-3.18c.78-.78.78-2.05 0-2.83L8.9 14.55l.71-.71 1.24 1.24c.78.78 2.05.78 2.83 0l1.41-1.41c.78-.78.78-2.05 0-2.83L13.84 9.6l.71-.71 1.24 1.24c.78.78 2.05.78 2.83 0l3.18-3.18c.78-.78.78-2.05 0-2.83L18.26.58c-.78-.78-2.04-.78-2.82.01zM6.6 19.32l-1.06 1.06L2 16.85l1.06-1.06 3.54 3.53zm2.12-2.12-1.06 1.06-3.54-3.54 1.06-1.06 3.54 3.54zm9.54-9.54L17.2 8.72l-3.54-3.54 1.06-1.06 3.54 3.54zm2.12-2.12L19.32 6.6l-3.54-3.54L16.85 2l3.53 3.54zM14 21v2c4.97 0 9-4.03 9-9h-2c0 3.87-3.13 7-7 7zm0-4v2c2.76 0 5-2.24 5-5h-2c0 1.66-1.34 3-3 3z'></path>
    </svg>
  );
}

const Air32 = () => {
  return (
    <svg
      height="24px"
      width="24px"
      viewBox='0 0 24 24'
      data-testid='AirIcon'
    >
      <path fill='#2288FF' d='M14.5 17c0 1.65-1.35 3-3 3s-3-1.35-3-3h2c0 .55.45 1 1 1s1-.45 1-1-.45-1-1-1H2v-2h9.5c1.65 0 3 1.35 3 3zM19 6.5C19 4.57 17.43 3 15.5 3S12 4.57 12 6.5h2c0-.83.67-1.5 1.5-1.5s1.5.67 1.5 1.5S16.33 8 15.5 8H2v2h13.5c1.93 0 3.5-1.57 3.5-3.5zm-.5 4.5H2v2h16.5c.83 0 1.5.67 1.5 1.5s-.67 1.5-1.5 1.5v2c1.93 0 3.5-1.57 3.5-3.5S20.43 11 18.5 11z'></path>
    </svg>
  );
}

const Source32 = () => {
  return (
    <svg
      height="24px"
      width="24px"
      viewBox='0 0 24 24'
      data-testid='SourceIcon'
    >
      <path fill='#FF828C' d='M20 6h-8l-2-2H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V8c0-1.1-.9-2-2-2zm-6 10H6v-2h8v2zm4-4H6v-2h12v2z'></path>
    </svg>
  );
}

export {
  BuildCircleIcon,
  SearchIcon24,
  Schedule32,
  CodeSnippet32,
  Bookmarks32,
  Notification32,
  CalendarToday32,
  Satellite32,
  Air32,
  Source32,
};
